...
class Agent:
    ...
    def _move_as_nomal(self, other_agents_positions):
        ...
        if best_nomal_vector_set['nomal_vector'] is None:
            ...
            self.position_history.append(self.position_history[-1].copy() + best_nomal_vector_set['nomal_vector'] * self.step_length)
...
class SimulationPlatform:
    ...
    def _visualize(self, update_times):
        ...
        edge_collection = LineCollection([], colors='gray', linewidths=0.8, alpha=0.35)
        ax.add_collection(edge_collection)
        text_labels = [
            ax.text(0, 0, str(i), fontsize=8, color='black', ha='left', va='bottom')
            for i in range(self.N)
        ]
        ...
        def animate(t):
            ...
            for idx, agent in enumerate(self.agents):
                ...
                text_labels[idx].set_position((x_pos + 0.3, y_pos + 0.3))
            ...
            for i in range(self.N):
                for j in self.visible_graph[i]:
                    if i < j:
                        segments.append([
                            self.agents[i].position_history[t],
                            self.agents[j].position_history[t],
                        ])
            edge_collection.set_segments(segments)
            time_text.set_text(f'Time: {t}')

            return scat_nomal, scat_adversal, edge_collection, leg, time_text, *text_labels
